...
def train(config, net, train_loader, optimizer, scheduler, history, kbar):
    ...
    for i, data in enumerate(train_loader):
        ...
        reg_loss = reg_loss_ra + reg_loss_rd #  try  reg_loss_ra + reg_loss_rd*3
...
def objective(trial, config, resume):
    ...
    optuna_para_config = {
        "optimizer": {
            "lr": trial.suggest_float("lr", 1e-4, 5e-3, log=True),#("lr", 1e-5, 1e-3, log=True), # original: 1-e4
    ...
    if batch_size == 4 or batch_size == 6  or batch_size == 8 or batch_size == 10:
        num_epochs = 100# 150
    ...
    for epoch in range(startEpoch, num_epochs): # num_epochs
...
if __name__ == '__main__':
    ...
    study = optuna.create_study(direction='maximize', study_name='FFTRadNet_optimization', 
                                ...
                                load_if_exists = True,
                                pruner=optuna.pruners.PercentilePruner(50.0, n_startup_trials=1,
    ...
    study.add_trial(optuna.create_trial(
        ...
        params=fixed_params,
        distributions={
            "lr": optuna.distributions.FloatDistribution(1e-4, 5e-3, log=True),#(1e-5, 1e-3, log=True),
