...
from model.FFTRadNet_ViT_doppler import FFTRadNet_ViT
from model.FFTRadNet_ViT_doppler import FFTRadNet_ViT_ADC
...
from loss.loss_doppler import pixor_loss
from utils.evaluation_doppler import run_evaluation
...
def objective(trial, config, resume):
    ...
    if config['data_mode'] != 'ADC':
        ...
        net = FFTRadNet_ViT_ADC(patch_size = config['model']['patch_size'],
                        ...
                        num_heads = config['model']['num_heads'],
                        drop_rates = config['model']['drop_rates'],
                        regression_layer = 3, # predict range, angle, doppler
                        detection_head = config['model']['DetectionHead'],
