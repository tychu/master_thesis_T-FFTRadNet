...
def train(config, net, train_loader, optimizer, scheduler, history, kbar):
    ...
    running_loss, running_cls_loss, running_reg_loss, running_mv_loss = 0.0, 0.0, 0.0, 0.0
    ...
    for i, data in enumerate(train_loader):
        ...
        # Apply loss weights and compute total loss
...
def objective(trial, config, resume):
    ...
    # Define optimizer, scheduler, and batch size
    ...
    num_epochs = 100 if batch_size <= 8 else 200

    # Initialize training history and data loaders
    ...
    for epoch in range(num_epochs):
        ...
        # Calculate F1 score
        F1_score = (
            (eval['mAP'] * eval['mAR']) / ((eval['mAP'] + eval['mAR']) / 2)
            if eval['mAP'] + eval['mAR'] > 0 else 0
        )
        ...
        # Save checkpoints after a certain number of epochs
        if epoch >= 20:
            checkpoint = {
            ...
            checkpoint_path = output_folder / exp_name / f"epoch_{epoch}.pth"
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--resume', default=None, type=str, help='Checkpoint file to resume training')
    ...
    # Define fixed parameters for the first trial
    fixed_params = {"lr": 1e-3, "step_size": 10, "batch_size": 4}
