...
def RA_to_cartesian_box(data):
    ...
    for i in range(len(data)):
        ...
        doppler = data[i][2]
        #print("doppler in RA_to_cartesian_box: ", doppler)

        boxes.append([x - W/2,y,x + W/2,y, x + W/2,y+L,x - W/2,y+L, doppler])
...
def process_predictions_FFT(batch_predictions, confidence_threshold=0.1, nms_threshold=0.05):
    ...
    #print("batch_predictions: ", batch_predictions.shape) # (i, 3) # row: # of targets; column: R A D C
    ...
    #print("batch_predictions: ", batch_predictions[0, :])
...
def GetFullMetrics(predictions,object_labels,range_min=5,range_max=100,IOU_threshold=0.5):
    ...
    for threshold in np.arange(0.1,0.96,0.1):
        ...
        for frame_id in range(len(predictions)):
            ...
            labels = object_labels[frame_id] # data[2] pd.read_csv(csv_file).to_numpy()
            #print("labels: ", labels.shape) # [3, 3] 
            ...
            if len(ground_truth_box_corners)>0 and len(Object_predictions)>0:
                ...
                for pid, prediction in enumerate(Object_predictions):
                    ...
                    if(len(ids)>0):
                        ...
                        angle_error += np.sum(np.abs(ground_truth_box_corners[ids,-2] - prediction[-2])) # prediction[-2]: x - W/2; gt_x -W/2 - (pre_x -W/2) = gt_x - pre_x
                        doppler_error += np.sum(np.abs(ground_truth_box_corners[ids,-1] - prediction[-1])) # prediction[-1]: y+L = gt_y - L - (pre_y - L) = gt_y - pre_y
                        nbObjects+=len(ids)
                    else:
                        FP+=1
                FN += np.sum(used_gt==0)
        ...
        if nbObjects != 0:
            ...
            DopplerError.append(doppler_error/nbObjects)
        else:
            continue       
    ...
    with open(output_file, 'a') as f:
        ...
        f.write('  Doppler Error: {0}\n'.format(np.mean(DopplerError)))      
    ...
    return [IOU_threshold,np.mean(perfs['precision']),np.mean(perfs['recall']),F1_score,np.mean(RangeError),np.mean(AngleError),np.mean(DopplerError)]
...
class Metrics():
    ...
    def update_RD(self, pre_obj_map, pre_RD, threshold):

        # pre_obj_map map[0, :, :]: cls 
        #print("pre_obj_map: ", pre_obj_map.shape)
        RD_gorund_truth_map = pre_obj_map[0, :, :]
        # pre_RD [cls, range, doppler]
        #print("pre_RD: ", pre_RD.shape)
        RD_predict_map = pre_RD[0, :, :]
        RD_binary_map = (RD_predict_map >= threshold).astype(int)
        # confusion matrix
        tn, fp, fn, tp = confusion_matrix(RD_gorund_truth_map.flatten(), RD_binary_map.flatten()).ravel()
        #print("fp, fn, tp: ", fp, fn, tp)
        self.RD_tp += tp
        self.RD_fp += fp
        self.RD_fn += fn

    def GetMetrics_RD(self,):
        
        if(self.RD_tp+self.RD_fp+self.RD_fn!=0):
            self.miou_RD = self.RD_tp / (self.RD_tp+self.RD_fp+self.RD_fn)
        else:
            self.miou_RD = 0
        
        if(self.RD_tp+self.RD_fp != 0):
            precision = ( self.RD_tp / (self.RD_tp+self.RD_fp)) # When there is a detection, how much I m sure
        else:
            precision = 0
        if(self.RD_tp+self.RD_fn != 0):     
            recall = (self.RD_tp / (self.RD_tp+self.RD_fn))
        else:
            recall = 0
        ...
        return self.miou_RD, precision, recall, f1
