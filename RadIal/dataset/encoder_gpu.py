...
class ra_encoder():
    ...
    def decode(self,map,threshold):
        ...
        # Ensure the input map is a tensor and move it to GPU if not already
        #map = map.to(device='cuda' if torch.cuda.is_available() else 'cpu')
        
        # Apply threshold and get the indices of the map where the value is greater than or equal to threshold
        range_bins, angle_bins = torch.where(map[0, :, :] >= threshold)

        # Determine the number of coordinates to process
        num_bins = range_bins.size(0)

        # Initialize an empty tensor to store coordinates on GPU
        coordinates_tensor = torch.empty((num_bins, 3), device=map.device)

        # Iterate through each bin and calculate coordinates directly using tensor operations
        for i in range(num_bins):
            range_bin = range_bins[i]
            angle_bin = angle_bins[i]

            # Calculate R, A, and C without using .item() to keep everything on GPU
            R = (range_bin * 4 * self.geometry['resolution'][0] +
                map[1, range_bin, angle_bin] * self.statistics['reg_std'][0] +
                self.statistics['reg_mean'][0])
            
            A = ((angle_bin - self.OUTPUT_DIM[2] / 2) * 4 * self.geometry['resolution'][1] +
                map[2, range_bin, angle_bin] * self.statistics['reg_std'][1] +
                self.statistics['reg_mean'][1])
            
            C = map[0, range_bin, angle_bin]
            
            # Assign calculated R, A, C to the tensor
            coordinates_tensor[i] = torch.tensor([R, A, C], device=map.device)
