...
class MATLAB(Dataset):
    ...
    def __init__(self, root_dir,folder_dir, statistics=None,encoder=None,difficult=False,perform_FFT='Default'):
        ...
        self.csv_files = [
                f for f in os.listdir(os.path.join(self.root_dir, self.folder_dir, 'ground_truth_with_vel/')) 
                if re.match(r'ground_truth_with_velocity_\d+\.csv', f)
            ]
    ...
    def __getitem__(self, idx):
        ...
        idx_adj = idx +1
        # load lables
        csv_file = os.path.join(self.root_dir, self.folder_dir, 'ground_truth_with_vel/',f"ground_truth_with_velocity_{idx_adj}.csv")
